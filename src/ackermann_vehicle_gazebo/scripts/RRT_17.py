...
XDIM = 25
YDIM = 25
delta = 0.2 # yeni düğüm için yakınlık
GOAL_RADIUS = 1.0
...
def step_from_to(p1, p2):  # eğer p2 noktası p1 noktasına yakınsa onu döndürür
    if dist(p1, p2) < delta:  # delta değeri 0.5
        return p2
    else:
        theta = atan2(p2[1] - p1[1], p2[0] - p1[0])
        return p1[0] + delta * cos(theta), p1[1] + delta * sin(theta)

def get_random_clear(): # Belirli sınırlar içinde random değerler üretir.
    while True:
...
def main(x, y):
    ...
    rate = rospy.Rate(0.1) # ROS Rate at 0.1 Hz
    ...
    while True:
        if currentState == 'init':
            ...
            bol_1.publish(True) # aracı çalıştır 
            rate.sleep() # 5 saniyede bir defa döngüye girer 
            bol.publish(True) # yeni verileri al 
            break
            
        elif currentState == 'buildTree':  # ağaç oluşturma kısmı
            count = count + 1
            if count < NUMNODES:  # eğer sayaç 15000 den küçükse ağaç oluşturmaya devam et
                ...
                print("Ran out of nodes... :(")  # sayaç değeri yeterli gelmedi ise bulunamadı mesajı yayımla
                return
...
def location_array_callback(data):
    ...
    if(x_cordinate[-1] == float(999.0) and y_cordinate[-1] == float(999.0)): # eğer gelen verinin sonu geldi ise 
        ...
        if data_complate == True: # eğer data geldi ise 
            ...
            bol_1.publish(msg) # robotu durdur çünkü hesaplama yapılıyor 
            print ("\ndata geldi")
            main(x_cordinate, y_cordinate) # hesaplama yapılır 
            x_cordinate = []
            y_cordinate = []
    else:
        data_complate = False
...
if __name__ == '__main__':
    ...
    bol = rospy.Publisher('engel_konumu_paylas', Bool, queue_size=10) # verinin alındığı ile alakalı uyarı mesajı yayımlar
    bol_1 = rospy.Publisher('start/stop', Bool, queue_size=10) # verinin alındığı ile alakalı uyarı mesajı yayımlar
    cmd = rospy.Publisher('rrt', Float32MultiArray, queue_size=10)
    engel = rospy.Subscriber('location', Float32MultiArray, location_array_callback) # bu topik engellerin konumu dizi olarak alır. ilk elemanı arabanın konumu 
    rospy.spin()
