...
def get_goal_direction():
    ...
    #print "hedefe dön"
    if regions_['onfull'] >= 2.0: # eğer etrafta bir engel yoksa yörüngeyi takip et
        if yaw_err < -0.1:
        ...
        if regions_['front'] > 2.0 and (regions_['right'] < 1.0 and regions_['left'] < 1.0) :
            cur_action = 'gf'
            next_action_time = rospy.Time.now() + rospy.Duration(linear_unit / linear_speed)
        elif regions_['front'] < 1.0:
            if regions_['bright'] > 1.5 and regions_['bleft'] > 1.5:
                cur_action = 'back'
                next_action_time = rospy.Time.now() + rospy.Duration(linear_unit / linear_speed)        
        else:
            if regions_['right'] < regions_['left']: # sol taraf boş ise
                cur_action = 'tl'
                next_action_time = rospy.Time.now() + rospy.Duration(linear_unit / linear_speed)
            elif regions_['right'] > regions_['left']: # sağ taraf boş ise:
                cur_action = 'tr'
                next_action_time = rospy.Time.now() + rospy.Duration(linear_unit / linear_speed)


def rrt_callback(data):
    global x_cordinate, y_cordinate, x, y
    ...
    if x_cordinate[-1] == 999.0: # eğer verinin tamamı geldi ise 
        ...
        if get_distance_between(current_pos, car_goal) > 2.0:
            main_fonk(path_plan) # hedef verildi
        else: 
        ...
        for p in data:
            ...
            while get_distance_between(cur_pos, q_goal) > goal_tolarence and not rospy.is_shutdown():
                ...
                if cur_action == 'tr':
                    ...
                    ackerman.steering_angle = 0.0
                    ackerman.steering_angle = angular_speed + 0.35 # geriye dönerken yönünü daha fazla çevirir
                    ackerman.speed = -linear_speed
                elif cur_action == 'rgf':
                    ackerman.steering_angle = 0.0
                    ackerman.steering_angle = -angular_speed - 0.35 # geriye dönerken yönünü daha fazla çevirir
                    ackerman.speed = -linear_speed
                elif cur_action == 'back':
                    ackerman.steering_angle = 0.0
                    ackerman.speed = -linear_speed
                else:
                    print("[Hata Durumu]") # önüne bir anda çıkan engel durumunda buraya girebiliyor. 
                    break
