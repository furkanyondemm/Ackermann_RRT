...
class Control():
    def __init__(self):
        ...
        global derece # derece 0/180 ve -180 arasında degerler alır. 
        global distance
        global paylas
...
if __name__ == '__main__':
    ...
    bol = rospy.Subscriber('engel_konumu_paylas', Bool, bool_callback)
    ...
    def clbk_laser(msg):
        ...
        for i in range(0, 720, 15):
            #distance.append(min(min(msg.ranges[i:i + 15]), 'nan')) for Melodic
            distance.append(min(msg.ranges[i:i + 15])) 
            ...
            if i <= 11:
                ...
                if (not math.isinf(-a)):
                    if (not math.isinf(b)):
                        ...
                        liste.append(tuple)
                        array_publish(liste[-1])
        tuple = (999.0, 999.0) # dizinin bitiş işareti olması için bu sayı gönderildi
        liste.append(tuple)
        array_publish(liste[-1])
        print('veri paylaşıldı! boyut: ' + str(len(liste)))
